      ...
      <Card>
        ...
        <h2 className='text-xl font-black text-white text-center'>Current Holder Details</h2>
          <div className="grid grid-cols-1 xxxlg:grid-cols-[1fr_2fr_1fr_1fr] gap-4 text-center text-lg pb-4">
            ...
            {
              (user?.role === "admin" && docData?.custody.currentHolder.role) && (<Button className='hover:text-black hover:bg-white cursor-pointer transition-all duration-400 xxxs:mb-5 sm:my-0 mx-auto font-black' onClick={()=>router.push(`/${lang}/users/${docData?.custody?.currentHolder?._id}`)}>Submission History</Button>)
            }
            {
              ((user?.role === "admin" && docData?.custody.currentHolder.adminLevel) && (<Button className='hover:text-black hover:bg-white cursor-pointer transition-all duration-400 xxxs:mb-5 sm:my-0 mx-auto font-black' onClick={()=>router.push(`/${lang}/admins/${docData?.custody?.currentHolder?._id}`)}>Reviews History</Button>))
            }
            {/* </CardContent> */}
        </CardHeader>
              ...
              <>
                ...
                <div className="tab-content bg-base-100 border-base-300 xxxs:px-1 xxxs:py-2 xs:p-6 bg-card rounded-xl text-center">
                  <div className="grid md:grid-cols-[1fr_2.5fr_1fr_0.5fr] sm:grid-cols-[1fr_1.5fr] xxxs:grid-cols-1 w-full max-w-full gap-2">
                    ...
                    {
                      (user?.role === "admin" && PreviousHolder.role) && (<Button className='hover:text-black hover:bg-white cursor-pointer transition-all duration-400 xxxs:mb-5 sm:mb-0 mt-6 text-center font-black' onClick={()=>router.push(`/${lang}/users/${PreviousHolder._id}`)}>Submission History</Button>)
                    }
                    {
                      ((user?.role === "admin" && PreviousHolder.adminLevel) && (<Button className='hover:text-black hover:bg-white cursor-pointer transition-all duration-400 xxxs:mb-5 sm:mb-0 mt-6 text-center font-black' onClick={()=>router.push(`/${lang}/admins/${PreviousHolder._id}`)}>Reviews History</Button>))
                    }
                </div>
              </>
              ))
