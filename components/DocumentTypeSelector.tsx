  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 xxs:grid-cols-2 gap-4">
      {templates.map((tpl : any) => (
        ...
        >
          <h3 className="text-xl font-black sm:text-xl px-3 py-3 font-sans">{lang === "ar" ? tpl.templateKeyAr : formatKeyToLabel(tpl.templateKey)}</h3>
          
          <div className="font-black font-sans flex gap-5 sm:flex-row sm:items-center sm:justify-center xxxs:flex-col xxxs:items-center xxxs:justify-center">
            {tpl.states.map((state : any) => (
              ...
              >
                {
                  lang === "en" ? state : (state === "Domestic" ? "محلي" : state === "Imported" ? "مستورد" : "عام") 
                }
              </Button>
            ))}
          </div>
